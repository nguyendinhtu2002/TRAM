...
function ProductCategory   () {
                ...
                >
                  <li className="flex items-center">
                    <input
                        ...
                        // name="country"
                        type="checkbox"
                        checked={origin === "Việt Nam"? true : false}
                        value="Việt Nam"
                  ...
                  <li className="flex items-center">
                    <input
                        ...
                        type="checkbox"
                        value="Philippines"
                        checked={origin === "Philippines"? true : false}
  ...
  switch (filterValue) {
    case "popularity":
      // Sắp xếp theo mức độ phổ biến (số lượng đánh giá)
      filteredData = data.slice().sort((a, b) => b.reviews.length - a.reviews.length);
      break;
    case "rating":
      ...
      break;
      // case "newest":
      //     // Sắp xếp theo sản phẩm mới nhất (chưa thấy biến date)
      //     filteredData = data.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
      //     break;
    case "priceLowToHigh":
      // Sắp xếp theo giá: thấp đến cao
      filteredData = data.slice().sort((a, b) => a.priceReal - b.priceReal);
      break;
    case "priceHighToLow":
      // Sắp xếp theo giá: cao đến thấp
      filteredData = data.slice().sort((a, b) => b.priceReal - a.priceReal);
      break;
    default:
      filteredData = data;
      break;
  ...
  if (origin !== "") {
    filteredData = filteredData.filter((product) => product.origin === origin);
  }
  console.log(origin)
  return (
      <>
          ...
          <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 sm:py-12 lg:max-w-7xl lg:px-8">
            ...
            <div className="w-full mb-8 flex justify-end">
              <div className="flex gap-3 items-center">
                <label htmlFor="filter" className="font-bold uppercase">
                  Sắp xếp
                </label>
                <select
                ...
                >
                  <option className="sm:text-sm" value="popularity">
                    Theo mức độ phổ biến
                  </option>
                  <option className="sm:text-sm" value="rating">
                    Theo điểm đánh giá
                  </option>
                  <option className="sm:text-sm" value="newest">
                    Mới nhất
                  </option>
                  <option className="sm:text-sm" value="priceLowToHigh">
                    Thứ tự theo giá: thấp đến cao
                  </option>
                  <option className="sm:text-sm" value="priceHighToLow">
